  ...
  fields: {
    style: {
      ...
      options: [
        ...
        { value: "archival-collage", label: "Muted archival collage", fragment: "Render as an artistic architectural photomontage assembled from real photographic pieces sourced from many different places. Make each surface — the floor, walls, ceiling, and every material texture — an actual photographic texture taken from a different source photo, so the timber floor, plaster walls, stone, and ceiling each clearly come from separate images with their own original character, fitted into the planes of the space. Render the furniture and objects as photographic cut-out assets that look found elsewhere and dropped into the scene — real photographed pieces composited in, not modelled or drawn. Fit and place these pieces only roughly, letting slight mismatches in scale, angle, and lighting show, so the image reads as a deliberate, hand-assembled compositional collage of disparate photographic sources rather than a seamless single photo or a CGI render. Then unify everything with one consistent muted grade: a soft near-monochrome greyscale with a faint warm undertone, gently and evenly desaturated, so all the mismatched pieces share the same tone, light, and exposure. Keep every piece clean and intact — not aged, damaged, dilapidated, torn, dusty, or distressed; the collaged quality should come from combining different sources, not from wear. Add a fine, subtle, even film grain across the whole image and only an almost-imperceptible vignette. The result should feel like a calm, atmospheric artist's photo-collage — quiet and tonal, and clearly assembled from many different photographic sources." },
