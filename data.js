...
/* Appended to every prompt — keeps the AI from adding drawing-sheet clutter. */
const NO_ANNOTATIONS =
  "Do not add any text, lettering, words, numbers, labels, captions, titles, dimensions, annotations, " +
  "callouts, leader lines, arrows, pointers, scale bars, north points, keys, legends, watermarks, logos, " +
  "borders, frames, or architectural title-block or drawing-sheet elements anywhere in the image; " +
  "produce only the image itself with no added markup.";
  ...
  fields: {
    style: {
      ...
      options: [
        ...
        { value: "archival-collage", label: "Muted archival collage", fragment: "Render as an artistic architectural photomontage assembled from real photographic pieces sourced from many different places, and make it clearly obvious that each element is a cut-out lifted from a magazine. Make each surface — the floor, walls, ceiling, and every material texture — a photographic texture taken from a different source image, and render the furniture and objects as magazine-cutout assets with the slightly flattened, printed quality of magazine paper. Make the cut-out edges slightly irregular, uneven, and a little random from piece to piece rather than perfectly clean or uniform. Make the cut-out elements slightly transparent, and composite them with interesting blend modes — favouring darkening and multiply blends — so overlapping pieces tint, deepen, and bleed into one another where they cross. Do not bring much linework or outline into the image; let the composition, overlap, and juxtaposition of the elements and textures describe the form and edges, rather than drawn lines. Fit and place these pieces roughly and deliberately: subtly adjust and warp the perspective of individual cut-outs so they are forced to sit into the space, let some elements completely ignore the lighting and shading of the scene — placed boldly and flatly as pure cut-out textures and objects — and let the forms be slightly imperfect and loosely defined. Layer many rich but very subtle textures over every element — printed halftone dots, paper grain, faint scan lines, and loose brush-like marks and painterly washes — and drift several soft, abstract mist passes across the scene, so each piece reads as a much-scanned, much-processed magazine fragment softened by paint and mist. Work primarily in black and white: keep the whole picture overwhelmingly desaturated, pale, and washed out — a predominantly greyscale, tonal image — and let only the faintest hint of colour come through, very subtle and barely-perceptible washed-out dusty blues and soft earthy tones, so muted they are almost imperceptible. Keep every piece clean and intact — not aged, damaged, dilapidated, torn, or dusty; the collaged quality should come from cutting, layering, blending, and processing different sources, not from wear. Add a fine, subtle, even film grain across the whole image and only an almost-imperceptible vignette. The result should feel like a calm, atmospheric artist's photo-collage — quiet, almost black-and-white, texturally layered, and clearly assembled from many different, heavily processed magazine cut-outs." },
        ...
        { value: "inkwork", label: "Architectural inkwork", fragment: "Render as a hand-drawn architectural ink perspective sketch, in the style of a classic architect's presentation drawing. Work strictly in black ink on white paper — monochrome line work with absolutely no colour at all, not even tints or washes. Draw with a slightly thick, organic, hand-drawn ink line that has a gentle, very subtle waver rather than being perfectly straight or mechanical — confident but human, as if drawn freehand with a fountain or fineliner pen. Let the line weight vary subtly and sit a touch heavier where each stroke starts and stops, where the ink pools slightly. Use mostly single-line contours with the occasional doubled, sketchy 'correction' line for energy. Set the scene out in precise one- or two-point perspective with accurate vanishing lines for windows, floors, ceiling beams, and stairs. Render structural elements — glazing, mullions, beams, stairs — crisply and accurately; draw furniture more loosely but in correct proportion; and give planting and foliage the most textured, organic linework. Use light hatching or crosshatching only sparingly — for foliage, the shadow beneath furniture, or window mullions — and leave most of the drawing as clean white paper. Include a few minimal, gestural, near-faceless figures for scale and a lived-in feel. Keep the mood calm, domestic, and daylight-filled, emphasising the connection to the outside through large glazing. It should read as a technical yet human, inviting architect's sketch — precise enough to convey real space and construction, loose enough to feel hand-drawn." },
