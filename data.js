  ...
  fields: {
    style: {
      ...
      options: [
        ...
        { value: "archival-collage", label: "Muted archival collage", fragment: "Render as an artistic architectural photomontage assembled from real photographic pieces sourced from many different places, in the manner of highly processed, heavily scanned magazine cut-outs. Make each surface — the floor, walls, ceiling, and every material texture — a photographic texture taken from a different source image, and render the furniture and objects as scanned magazine-cutout assets that look found elsewhere and dropped into the scene, not modelled or drawn. Fit and place these pieces roughly and deliberately: subtly adjust and warp the perspective of individual cut-outs so they are forced to sit into the space, and let some elements completely ignore the lighting and shading of the scene — placed boldly and flatly as pure cut-out textures and objects, regardless of the room's light direction. Layer many very subtle textures, grains, and processing passes over the elements, so each one reads as a much-scanned, much-copied magazine fragment. Use colour sparingly and very washed out: a muted, faded palette led by soft dusty blues, earthy tones, and off-white, with only occasional restrained hints of other colour, all knocked back and unified so the mismatched pieces share the same processed, low-saturation tone. Keep every piece clean and intact — not aged, damaged, dilapidated, torn, or dusty; the collaged quality should come from combining and processing different sources, not from wear. Add a fine, subtle, even film grain across the whole image and only an almost-imperceptible vignette. The result should feel like a calm, atmospheric artist's photo-collage — quiet, washed-out, and clearly assembled from many different, heavily processed photographic sources." },
